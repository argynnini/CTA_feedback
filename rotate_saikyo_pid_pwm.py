...
test_resistance = [0.39 * math.sqrt(25.36 - x) + 7.4 for x in test_distance] # テスト抵抗値
...
try:
    ...
    print("テスト抵抗値: ", test_resistance, "予想時間: ", len(test_resistance) * test_interval, "秒")
    seikaku_count = 0
    ...
    while time.perf_counter() - start < len(test_resistance) * test_interval:
        pid.setpoint = target_resistance = test_resistance[int((time.perf_counter() - start) / test_interval) % len(test_resistance)]
        ...
        # 情報開示
        sa = ((f'\033[32m' if abs(resistance - target_resistance) < 0.1 else
              f'\033[34m' if resistance > target_resistance else
              f'\033[31m') +
              f'{resistance:02.2f}\033[0m/{target_resistance:02.2f}')
        print(f'S:{elapse:4.2f}s', f'\033[32m{seikaku_count}\033[0m',
              f'P:{pid.components[0]:+03.1f}', f'I:{pid.components[1]:+03.1f}', f'D:{pid.components[2]:+03.1f}',
              f'PWM:{int(output):03d}',
              f'OHM:{sa}Ω', end="\r")

        # CSVキャッシュ書き込み
        csv_data = np.concatenate((csv_data, [[elapse, *mean, resistance, target_resistance, output]]), axis=0)
    ...
    if isSave:
        ...
        np.savetxt(f'{work_dir}/{csv_filename}.csv', np.array(csv_data), delimiter=',', header= ','.join (csv_header) + f',#Kp:{Kp},#Ki:{Ki},#Kd:{Kd},#avg:{read_samples},#freq:{pwm_freq},#duty:[{duty_range[0]}-{duty_range[1]}],#Vth:{Vthreshold},{now.strftime("%Y/%m/%d-%H:%M:%S")}', comments='')
